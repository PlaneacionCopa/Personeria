...
import type { Atencion, NuevaAtencionInput, Tipificaciones, Usuario } from "../types/atencion";
...
export default function Create() {
  ...
  const [casosPrevios, setCasosPrevios] = useState<Atencion[]>([]);
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        ...
        <p className="mt-1 text-sm text-slate-600">
          La hora de atención y el tiempo de espera se calculan automáticamente al guardar. Si la
          persona ya tiene un caso abierto por el MISMO asunto, mejor agrega una visita al hilo
          desde "Buscar" en vez de crear un caso nuevo aquí.
        </p>
        ...
        <Card className="mt-6 p-5">
          ...
          {casosPrevios.length > 0 && (
            <div className="mb-4 rounded-xl border border-amber-100 bg-amber-50 p-3 text-sm text-amber-800">
              Esta persona ya tiene {casosPrevios.length} caso(s) registrado(s):{" "}
              {casosPrevios.map((c) => c.asunto).join(", ")}. Si es por el mismo asunto, mejor
              agrega una visita al hilo desde "Buscar" en vez de duplicar el caso aquí.
            </div>
          )}
